...
class ThemeManager {
  ...
  setTitleBarStyle(style) {
    const newStyle = titleBarStyles[style];
    if (!newStyle) {
      return;
    }
  ...
  static applyCustomColors() {
    ...
    const colorBase = atom.config.get("fancy-bar.colors.baseColor");
    const colorHi = atom.config.get("fancy-bar.colors.highlightColor");
    const colorText = atom.config.get("fancy-bar.colors.textColor");
