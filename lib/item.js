...
class MenuItem {
  ...
  static createMenuItem(menuItem) {
    ...
    if (menuItem.command === "application:reopen-project" && menuItem.commandDetail?.paths?.length) {
  ...
  onMouseClick(e) {
    ...
    if (this.isExecutable()) {
      // Cache command info and target before closing menu
      const command = this.command;
      const commandDetail = this.commandDetail;
      // Cache target ONCE to avoid redundant getActiveTextEditor() calls
      const target =
        this.targetElement ||
      ...
      // Single RAF - menu is already closed, command can run immediately
      // requestAnimationFrame isn't enough for native pop-ups
      setTimeout(() => {
        if (command !== undefined) {
          if (exceptionCommands.has(command)) {
            ipcRenderer.send("command", command);
          } else {
            atom.commands.dispatch(target, command, commandDetail);
          }
        }
      }, 10);
    }
  ...
  async execCommand() {
    ...
    if (exceptionCommands.has(this.command)) {
      ipcRenderer.send("command", this.command);
      return;
    }
