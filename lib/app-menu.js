...
class ApplicationMenu {
  constructor(element, parent) {
    ...
    this.paneItemDisposable = atom.workspace.onDidChangeActivePaneItem((item) => this.blur());
  ...
  onKeyDown(e) {
    ...
    if (e.key === "Alt") {
      ...
      // Only show mnemonics if altGivesFocus or autoHide is enabled
      if (bar.configState.menuBarMnemonics && (bar.configState.altGivesFocus || bar.configState.autoHide)) {
      ...
      switch (e.key) {
        ...
        case "ArrowRight":
          ...
          if (!selected || !selected.hasSubmenu()) {
            ...
            selected.getParent()?.getSubmenu()?.forEach((o) => {
              if (o !== selected) {
                o.setOpen(false);
              }
            });
